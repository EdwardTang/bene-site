...
DEMO_EMBEDS = {
    "README": ("demo-keyless.gif", "Demo — keyless first value: bene init && bene demo --no-ui, 0.3s, recorded live against bene-main HEAD."),
    "cli-reference": ("demo-litany.gif", "Demo — rewind a bad turn: an agent scaled replicas 3 → 0; checkpoint → diff → restore puts it back."),
    "mcp-integration": ("demo-honesty.gif", "Demo — what a connected agent inherits: experiments ls / show + senses --md."),
}
...
def page(rel: Path, body: str, title: str, needs_mermaid: bool, is_index: bool = False) -> str:
    ...
    crumbs = "" if is_index else f'<div class="crumbs"><a href="{idx}">docs</a> / {html.escape(rel.as_posix())}</div>'
    src_line = "" if is_index else (
...
def build() -> None:
    ...
    for label, sub in GROUPS:
        ...
        groups_html.append(f'<div class="idx-group"><h2>{html.escape(label)}</h2><ul>{lis}</ul></div>')

    index_body = (
        "<h1>BENE documentation</h1>"
        f"<p>{len(entries)} documents, generated from <code>docs/</code> — the same files you can read "
    ...
    (OUT / "index.html").write_text(
        page(Path("index.md"), index_body, "BENE documentation", False, is_index=True), encoding="utf-8"
    )
