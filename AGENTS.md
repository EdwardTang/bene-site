...
- `docs/design/CLAIMS-AUDIT.md` — implemented vs planned, per claim. **Check before asserting a capability.**
...
2. Never present a CLAIMS-AUDIT "planned" item as working (no plasticity ranking, no critical-step
   localizer, no continuous-quality signal, no runner ContextOS/loop-guard wiring, no scheduled
   consolidation). The phrase "BENE supersedes" is banned by the benchmark report.
