...
> *Everything is an engram.* One typed, append-only, provenance-linked substrate
> under five capability pillars, with falsifiable gates on everything that
...
| Pillar | What ships in `bene/kernel/` |
| --- | --- |
| **Engram substrate** | compression ladder (raw trace → episodic → semantic → procedural → strategic), mandatory provenance, lineage queries, FTS |
...
```bash
uv run bene demo --no-ui     # the 5-pillar story
...
The meta-harness runs an evolutionary search over harness strategies. It evaluates candidate harnesses on a benchmark, stores traces in the agent VFS, and exposes frontier inspection commands.
