# How BENE AI Agents Found a Production Root Cause in 12 Seconds

*SRE / Operations*

*23% HTTP 500 rate at 2am. A BENE agent queried the append-only event journal, found 847 ConnectionPoolErrors, and traced them to a single config write 47 minutes prior — in 12 seconds.*
...
The BENE answer is a SQL query. It takes 12 seconds.

---

![BENE incident response demo — SQL query finds root cause in 12 seconds, hotfix drops error rate to 0%](../demos/bene_uc_incident.gif)

*Pager fires at 02:17. SQL query at 02:17:12. Root cause identified at 02:17:24. Fix deployed. Error rate: 23% → 0%.*

---

## Traditional Incident Response vs BENE
...
The question "what changed?" should always have a fast answer. BENE makes it a SQL query. The 12-second root cause lookup isn't a trick — it's what happens when every agent write is journaled in a queryable append-only log from the start.

## Related

- [README](../README.md) — BENE overview and full doc index
- [Use Cases](../use-cases.md) — more real-world patterns
- [Schema: events table](../schema.md#events)
- [Use case: 2am Incident Response](../use-cases.md#2am-incident-response)
