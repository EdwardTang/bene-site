...
> Breeding-program · Evolutionary · Nexus · Engrams — a Bene Gesserit harness

This document describes the internal architecture of BENE: its design philosophy, subsystem boundaries, data flow, and integration points.
...
BENE is a runtime framework for managing autonomous AI agents. Each agent receives an isolated, auditable virtual filesystem backed by a single SQLite database file. The system is composed of five major subsystems:
...
Everything an agent produces -- files, state, tool call logs, event history, checkpoints -- lives in one `.db` file. Copying, backing up, or transferring an agent runtime is `cp bene.db backup.db`. Any SQLite client can query the database.

### Zero AI SDK dependencies

BENE uses no `openai`, no `litellm`, no `dspy`. All communication with vLLM inference endpoints is done through raw `httpx` HTTP calls to the OpenAI-compatible `/v1/chat/completions` API. This eliminates SDK version conflicts, reduces the dependency tree, and keeps the attack surface minimal.
...
BENE subsystems are composed rather than subclassed. `Bene` owns a `BlobStore`, an `EventJournal`, and a `CheckpointManager`. The `ClaudeCodeRunner` delegates inference to the `TierRouter`. The MCP server delegates to both. Each subsystem can be tested, replaced, or extended independently.

---

## VFS Engine

The VFS Engine is the core of BENE, implemented in `bene/core.py` as the `Bene` class. It provides a virtual filesystem, state management, tool call tracking, and checkpoint/restore -- all backed by SQLite.
...
`Bene` uses `threading.local()` to maintain one SQLite connection per thread. This avoids the SQLite threading pitfalls while allowing true parallel agent execution in threaded environments.
...
Files are stored as content-addressable blobs in the `blobs` table (`bene/blobs.py`). The blob store provides:
...
The event journal (`bene/events.py`) is an append-only log of every significant action in the system. It records:
...
The checkpoint system (`bene/checkpoints.py`) captures point-in-time snapshots of an agent's complete state:
...
BENE provides two tiers of agent isolation, implemented in `bene/isolation.py`.

### Tier 1 -- Logical Isolation (Default)

Every VFS operation is scoped by `agent_id`. The `LogicalIsolation` class wraps `Bene` methods, binding a specific `agent_id` so the caller cannot accidentally (or intentionally) access another agent's data.
...
For environments requiring OS-level process isolation, BENE can mount each agent's VFS as a FUSE filesystem. The `IsolatedAgentProcess` class provides:

1. **FUSE mount**: The agent's virtual filesystem is mounted at `/tmp/bene/<agent_id>`. The agent process sees a standard filesystem -- it has no knowledge that reads and writes are backed by SQLite.
...
- `fusepy` package (`uv pip install bene[fuse]`).
...
The **Claude Code Runner** (CCR), implemented in `bene/ccr/runner.py`, orchestrates the agent execution loop. It implements a **plan-act-observe** cycle.
...
2. **Plan** (model inference): The conversation is sent to the Tier router, which classifies the task complexity and routes to the appropriate vLLM model. The model returns text content and/or tool calls.
...
The `ToolRegistry` (`bene/ccr/tools.py`) manages built-in and custom tools:
...
The **Difficulty-Aware Routing by Tier** (Tier) router (`bene/router/tier.py`) classifies each inference request's task complexity into one of `trivial / moderate / complex / critical` and dispatches to the matching model tier, with automatic fallback to a designated model on failure.
...
**LLM Classifier** (`bene/router/classifier.py: LLMClassifier`):
...
**Heuristic Classifier** (`bene/router/classifier.py: HeuristicClassifier`):
...
The routing table maps complexity levels to model names. It is built automatically from each model's `use_for` annotations in `bene.yaml`:
...
The `ContextCompressor` (`bene/router/context.py`) manages conversation history to fit within model context windows. It uses a multi-stage strategy:
...
The `VLLMClient` (`bene/router/vllm_client.py`) is a lightweight async HTTP client that speaks the OpenAI-compatible chat completions API:
...
BENE exposes its full functionality as an MCP (Model Context Protocol) server, enabling integration with Claude Code and other MCP-compatible clients.
...
The server exposes 17 tools across 6 categories (Lifecycle, VFS, Checkpoints, Query, Orchestration, Meta-Harness) that map directly to `Bene`, `ClaudeCodeRunner`, and `MetaHarnessSearch` operations. See [mcp-integration.md](mcp-integration.md) for the complete tool reference.
...
The server holds module-level references to `Bene` and `ClaudeCodeRunner`, which are set during initialization and used by all tool handlers.
