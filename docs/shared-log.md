...
The shared log is a monotonically-growing, append-only sequence that gives every agent in a BENE project a consistent view of collective intent, votes, and decisions — enabling safe multi-agent coordination without a central coordinator process.

![Shared Log — intent / vote / decide / commit across agents](demos/bene_uc_shared_log.gif)

> Inspired by **LogAct: Enabling Agentic Reliability via Shared Logs**
> Balakrishnan, Shi, et al. (2026), arXiv:2604.07988, Meta.
> Adapted for BENE's local-first, SQLite-backed architecture.
...
- Integrated with BENE agent lifecycle (agent_id as first-class citizen)
